...
class Cola_Helper_Tool {
    ...
    public static function Curl($destURL, $timeout = 0, $paramStr = '', $flag = 'get', $name = '', $password = '') {
        ...
        if ($timeout) {//修改为 CURLOPT_TIMEOUT_MS
            //curl_setopt($curl, CURLOPT_TIMEOUT, $timeout);
            //见 http://www.php.net/manual/vote-note.php?id=104597&page=function.curl-setopt&vote=up
        ...
        curl_setopt($curl, CURLOPT_DNS_CACHE_TIMEOUT, 60*60*24);
    ...
    public static function getVidByUrl($url) {
        if (empty($url) || strpos($url, 'http://www.letv.com/ptv/vplay/') === false)
            return false;
