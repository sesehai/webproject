 ...
 *
 * 使用方法：
 * $xssFilterObj = new Cola_Helper_XssFilter();
 * 
 * $xssFilterObj->disallow_http();//不保留http://xxx.xxx这种格式(默认)
 * $xssFilterObj->allow_http();//保留http://xxx.xxx这种格式
 * 
 * $xssFilterObj->disallow_htmlspecialchars(); //关闭执行字符转义
 * $xssFilterObj->allow_htmlspecialchars(); //开启执行字符转义（默认）
 * 
 * $xssFilterObj->filter_it($content);
 *
 * 
 * @author wangguanlin@letv.com
 * @copyright 2013 letv.com
 * @link www.letv.com
 */
class Cola_Helper_XssFilter 
{
