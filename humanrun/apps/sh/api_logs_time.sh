...
cat /usr/local/nginx/logs/api.yonggongbao.cn.log | awk '{if($7 != "/"){print $7, $(NF-1), $NF}}' | awk 'gsub(/"/,"",$0) {print $0}' | awk -F"?" '{print $1, $2}' | awk '{print $1,$3,$4}' | awk 'BEGIN {uamount[$1]=0;u_rows[$1]=0;print "接口,总耗时(毫秒),总行数,平均耗时(毫秒)"} {uamount[$1]=uamount[$1]+$(NF-1)*1000;u_rows[$1]=u_rows[$1]+1;} END{for(i in uamount){average=uamount[i]/u_rows[i];print i,",",uamount[i],",",u_rows[i],",",average;}}'
